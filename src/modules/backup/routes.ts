...
export function registerBackupRoutes(app: FastifyInstance): void {
  app.post(
    ...
    { preHandler: [authenticateVerified, firmAccess, requireRole('admin')], bodyLimit: BODY_LIMIT },
    async (req) => {
  ...
  app.get('/firms/:firmId/export/backup', { preHandler: [authenticateVerified, firmAccess] }, async (req, reply) => {
