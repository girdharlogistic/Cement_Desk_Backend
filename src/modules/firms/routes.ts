...
export function registerFirmRoutes(app: FastifyInstance): void {
  app.get('/firms', { preHandler: [authenticate] }, async (req) => svc.listFirms(req.userId));

  app.post('/firms', { preHandler: [authenticate] }, async (req, reply) => {
  ...
  app.patch(
    '/firms/:firmId',
    { preHandler: [authenticate, firmAccess, requireRole('admin')] },
    async (req) => {
  ...
  app.delete(
    '/firms/:firmId',
    { preHandler: [authenticate, firmAccess, requireRole('owner')] },
    async (req, reply) => {
  ...
  app.get('/firms/:firmId/members', { preHandler: [authenticate, firmAccess] }, async (req) =>
